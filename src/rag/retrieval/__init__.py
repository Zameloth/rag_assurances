...
adding the retriever observation span — is deliberately not built out here (see the #28 issue
comment: LangChain/Langfuse portions are paired, not agent-authored); it lives in
`langchain_retriever.py` instead, including the reranker's own hand-wrapped Langfuse span
(SPEC §11.1 — it doesn't auto-trace, not being a LangChain component). The quota lands with
